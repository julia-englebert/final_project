...
data1940j <- data1940j %>%
  ...
  mutate(value_farm = BWD001+BWD002, n_farm = BY1001, cows = NA, horses = NA, 
         ...
         acres_irrigated = NA, pct_irrigated = NA) %>%
  mutate(cows_per_person = cows/n_farm, pct_enrolled = enrolled/pop*100, 
...
data1950j <- full_join(data1950a, data1950b, by = c("STATEA", "COUNTYA")) %>%
  ...
  mutate(value_farm = B3M001, n_farm = B3Z001, cows = NA, horses = NA, 
         avg_size = B3L001, value_implements = NA, pop = B13001, male = B3E001, 
...
allData_inflated <- as_tibble(bind_rows(data1870j, data1880j, data1890j, 
  ...
  mutate(pct_male = male/pop*100, avg_value_farm = value_farm/n_farm, cows_per_farm = cows_per_person, 
...
```{r experimentalModels}
# USE subData FOR ALL MODELS
...
maleedu_plot <- maleedu_model %>%
  ...
  geom_errorbar(ymin = maleedu_model$pct_male_lower, ymax = maleedu_model$pct_male_upper) + 
  geom_point() +
  theme_classic() +
  ylim(-1.5, 0.5) + 
  labs(x = "Year",
       ...
       subtitle = "Measuring the effect of being male on school enrollment")

# avg_value_implements and cows_per_farm have an effect on enrollment
# in general, for 1879 and 1890, more cows per farm = less enrollment
# aka bigger farms = less enrollment?
model2a <- modelData %>% 
  filter(YEAR == "1870") %>%
  lm(data = ., pct_enrolled ~ cows_per_farm) %>% 
  tidy(conf.int = TRUE) %>% 
  select(term, estimate, p.value, conf.low, conf.high) %>%
  gt() %>% 
  fmt_number(data = ., columns = c("estimate", "p.value", "conf.low", "conf.high"), 
             decimals = 2)

# there was an infinite value left in the cows_per_farm column for this year
# not sure why, but I got rid of it
model2a <- modelData %>% 
  filter(YEAR == "1890", !is.infinite(cows_per_farm)) %>%
  lm(data = ., pct_enrolled ~ cows_per_farm) %>% 
  tidy(conf.int = TRUE) %>% 
  select(term, estimate, p.value, conf.low, conf.high) %>%
  gt() %>% 
...
# Enrollment/Illiteracy model (just for testing)

# This is saying that for every 1% increase in illiterate people, there is a 0.24% 
# decrease in school enrollment, which makes sense
model3 <- modelData %>%
  lm(data = ., pct_enrolled ~ pct_illiterate) %>%
  tidy(conf.int = TRUE) %>%
  select(term, estimate, p.value, conf.low, conf.high) %>%
  gt() %>%
  fmt_number(data = ., columns = c("estimate", "p.value", "conf.low", "conf.high"), 
...
mod3plot <- modelData %>%
  filter(! is.na(pct_enrolled) & ! is.na(pct_illiterate)) %>%
  ggplot(aes(pct_enrolled, pct_illiterate)) + 
  geom_point() + 
  geom_smooth(se = FALSE)
```
...
# can only use percentages until adjusting for inflation
# OR deal with each year independently 

# Set the NA values to almost zero
# Not ideal, but necessary for running regressions
# setting to zero returns NaNs

reg_years <- allData %>% 
  # sample the data to run faster
  # change this later
  sample_n(size = 5000) %>%
  # Get rid of the infinite cows_per_farm values
  filter(! (is.na(pct_enrolled) & is.na(avg_size))) %>%
  group_by(YEAR) %>%
  arrange(desc(YEAR)) %>% 
  nest() 
  
  # Call map inside of mutate to add new columns to the tibble
  # reg_results are the results of the regression we ran in question 2
  # coef is the coefficient of log_migrants for each continent
  # upper and lower are the upper and lower bounds of the confidence interval created by tidy()
  ...
  #STATEA, COUNTYA, GISJOIN, YEAR, STATE, COUNTY, value_farm, value_implements, avg_size, n_farm, 
         # acres_farms, acres_irrigated, pct_irrigated, cows, cows_per_farm, horses, pop, male, female, 
        # enrolled, pct_enrolled, urban_pop, pct_urban, illiterate, pct_illiterate
reg_years1 <- reg_years %>%
  mutate(mod = map(data, ~ lm(pct_enrolled ~ avg_size + avg_value_implements + n_farm + pct_irrigated + cows_per_farm + pct_urban, data = .)),
         reg_results = map(mod, ~ tidy(., conf.int = TRUE)),
         avg_size_coef = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% 
                                   pull(estimate)),
         avg_size_upper = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% 
                                    pull(conf.high)),
         avg_size_lower = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% 
                                    pull(conf.low)),
         avg_value_implements_coef = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% 
                                               pull(estimate)),
         avg_value_implements_upper = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% 
                                                pull(conf.high)),
         avg_value_implements_lower = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% 
                                                pull(conf.low)),
         n_farm_coef = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(estimate)),
         n_farm_upper = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(conf.high)),
         n_farm_lower = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(conf.low)),
         pct_irrigated_coef = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(estimate)),
         pct_irrigated_upper = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(conf.high)),
         pct_irrigated_lower = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(conf.low)),
         cows_per_farm_coef = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(estimate)),
         cows_per_farm_upper = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(conf.high)),
         cows_per_farm_lower = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(conf.low)),
         pct_urban_coef = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(estimate)),
         pct_urban_upper = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(conf.high)),
         pct_urban_lower = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(conf.low))
  )
  
# Now use the resulting tibble to create the gt table

q4_gt <- q4 %>%
  
  # The original year column was giving me problems because it's a grouping variable
  # Ungrouping solved the problem
  ...
  tab_header(title = "Title",
             subtitle = "With 95% Confidence Interval") %>%
  cols_label(YEAR = "Year", coef = "Migrant Coefficient", 
             lower = "Lower Bound", upper = "Upper Bound") %>%
  
  # Round all values to two decimal places
  
  fmt_number(columns = vars(coef, lower, upper), decimals = 2)
...
# PASTE DATA and LIBRARIES here after
