...
modelData <- allData %>%
  ...
  filter(! is.na(pct_enrolled) & pct_enrolled > 0) %>%
  group_by(YEAR)

ggplot(modelData, aes(x = avg_value_implements, y = pct_enrolled, color = YEAR)) + 
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm", se = FALSE) +
  ylim(0, 100) + 
  labs(title = "Relationship Between Farm Value and Education, by County",
       subtitle = "Midwestern United States", 
       x = "Avergae Value of Farms", 
       y = "Percent Population Enrolled in School") +
  scale_x_log10(labels = comma) +
  theme_classic()

model1 <- modelData %>% 
  filter(avg_value_farm > 0, avg_value_implements > 0, n_farm > 0) %>%
  lm(data = ., pct_enrolled ~ avg_value_farm + pct_urban + n_farm) %>% 
  tidy(conf.int = TRUE) %>% 
  gt() %>% 
...
# modelData$YEAR <- fct_explicit_na(modelData$YEAR)

model2b <- modelData %>% 
  filter(! is.na(cows_per_farm)) %>%
  lm(data = ., pct_enrolled ~ cows_per_farm) %>% 
  tidy(conf.int = TRUE) %>% 
  select(term, estimate, p.value, conf.low, conf.high) %>%
  gt() %>% 
...
reg_years <- allData %>% 
  ...
  filter(! is.infinite(cows_per_farm)) %>%
  group_by(YEAR) %>%
  arrange(desc(YEAR)) %>%
  
  # Nest to create a tibble of tibbles
...
mutate(mod = map(data, ~ lm(pct_enrolled ~ avg_size + avg_value_implements + n_farm + pct_irrigated + cows_per_farm + pct_urban, data = .)),
         ...
         coef = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "avg_size") %>% pull(conf.low)),
         coef = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "avg_value_implements") %>% pull(conf.low)),
       coef = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "n_farm") %>% pull(conf.low)),
       coef = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "pct_irrigated") %>% pull(conf.low)),
       coef = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "cows_per_farm") %>% pull(conf.low)),
       coef = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(estimate)),
         upper = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(conf.high)),
         lower = map_dbl(reg_results, ~ filter(., term == "pct_urban") %>% pull(conf.low))
         )
...
q4_gt <- q4 %>%
  ...
  tab_header(title = "Effect of Number of Migrants on Amount of Remittances by Year",
